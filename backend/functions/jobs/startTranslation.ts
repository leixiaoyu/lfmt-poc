...
export const handler = async (event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> => {
  ...
  try {
    ...
    // Parse request body
    const body: StartTranslationRequest = event.body ? JSON.parse(event.body) : {};
    ...
    if (job.translationStatus === 'IN_PROGRESS' || job.translationStatus === 'COMPLETED') {
      return createErrorResponse(
        400,
...
function validateRequest(body: StartTranslationRequest): {
  ...
  if (body.contextChunks !== undefined && (body.contextChunks < 0 || body.contextChunks > 5)) {
    return {
      valid: false,
 ...
 */
function calculateEstimatedCost(totalChunks: number, tokensPerChunk: number): number {
